...
export function classifyRegime(features: RegimeFeatures): MarketRegime {
  ...
  const atrPercentile = hasAtr ? features.atrPercentile : undefined;
  const bbWidthPercentile = hasBb ? features.bbWidthPercentile : undefined;
  const trending = (adx4h !== undefined && adx4h >= 25) || features.hasMarketStructureShift === true;
  const compressed = bbWidthPercentile !== undefined && bbWidthPercentile <= 0.2;
  const ranging = adx4h !== undefined && atrPercentile !== undefined && adx4h < 20 && atrPercentile < 0.6;
  const volatile = atrPercentile !== undefined && atrPercentile >= 0.8;
