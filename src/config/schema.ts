...
export const ConfigSchema = z.object({
  ...
  STALE_FLOOR_depthUpdate: z.coerce.number().int().positive().default(60000),
  ...
  SKEW_THRESHOLD_MS: z.coerce.number().int().positive().default(5000),
  ...
  LIQUIDITY_POOL_STRENGTH_DECAY: z.coerce.number().positive().max(1).default(0.95),
  LIQUIDITY_MAX_POOLS_PER_PAIR: z.coerce.number().int().positive().max(50).default(12),
  LIQUIDITY_MIN_PENETRATION_PCT: z.coerce.number().positive().max(2).default(0.05),
  LIQUIDITY_MAX_PENETRATION_PCT: z.coerce.number().positive().max(5).default(0.4),
  LIQUIDITY_PENETRATION_ATR_SCALE: z.coerce.number().nonnegative().max(5).default(1),
  LIQUIDITY_VELOCITY_WINDOW_MS: z.coerce.number().int().positive().max(30_000).default(800),
  LIQUIDITY_VELOCITY_MIN_PCT_PER_SEC: z.coerce.number().nonnegative().max(10).default(0.10),
  LIQUIDITY_VOLUME_SPIKE_MULT: z.coerce.number().positive().max(10).default(1.5),
  LIQUIDITY_VOLUME_LOOKBACK_BARS: z.coerce.number().int().positive().max(100).default(10),
  LIQUIDITY_MAX_REJECTION_BARS: z.coerce.number().int().positive().max(20).default(5),
  LIQUIDITY_ACCEPTANCE_HOLD_BARS: z.coerce.number().int().positive().max(10).default(2),
  LIQUIDITY_EVENT_MAX_AGE_MS: z.coerce.number().int().positive().default(3_600_000),
  LIQUIDITY_EVENT_MAX_BARS_SINCE_SWEEP: z.coerce.number().int().positive().max(30).default(8),
  LIQUIDITY_ACTIONABLE_SCORE_MIN: z.coerce.number().int().min(0).max(20).default(8),
  LIQUIDITY_WATCHLIST_SCORE_MIN: z.coerce.number().int().min(0).max(20).default(5),
  LIQUIDITY_STRUCTURE_MSS_BONUS: z.string().default('true').transform(s => s !== 'false'),
