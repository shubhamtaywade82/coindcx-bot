...
describe('AccountPersistence', () => {
  it('returns false when dedup insert does not insert row', async () => {
    const query = vi.fn(async () => ({ rowCount: 0 }));
    const persistence = new AccountPersistence({
      pool: { query } as any,
      retryMax: 10,
    });
    const accepted = await persistence.recordAccountEventDedup({
      clientOrderId: 'cid-1',
      eventId: 'evt-1',
      entity: 'order',
    });
  ...
  it('returns true when dedup insert is accepted', async () => {
  ...
  it('upsertPosition issues INSERT ... ON CONFLICT DO UPDATE', async () => {
    const f = fakePool();
    const persistence = new AccountPersistence({ pool: f.pool as any, retryMax: 100 });
    await persistence.upsertPosition(p1);
  ...
  it('appendFill is idempotent via ON CONFLICT DO NOTHING', async () => {
    const f = fakePool();
    const persistence = new AccountPersistence({ pool: f.pool as any, retryMax: 100 });
    await persistence.appendFill({
      id: 'f1',
      pair: 'X',
      side: 'buy',
      price: '1',
      qty: '1',
      executedAt: '2026-04-26T00:00:00Z',
      ingestedAt: '2026-04-26T00:00:01Z',
      source: 'ws',
    });
  ...
  it('recordChangelog inserts into account_changelog', async () => {
    const f = fakePool();
    const persistence = new AccountPersistence({ pool: f.pool as any, retryMax: 100 });
    await persistence.recordChangelog({
      entity: 'position',
      entityId: 'p1',
      field: 'markPrice',
      oldValue: '50100',
    ...
    expect(f.calls[0]!.params).toEqual(
      expect.arrayContaining(['position', 'p1', 'markPrice', '50100', '51000', 'ws_apply', null]),
    );
  });
  ...
  it('queues writes when pool throws and flushes on success', async () => {
    ...
    const persistence = new AccountPersistence({ pool: pool as any, retryMax: 100 });
    await persistence.upsertPosition(p1);
    expect(persistence.queueSize()).toBe(1);
    fail = false;
    await persistence.flush();
    expect(persistence.queueSize()).toBe(0);
    expect(calls.length).toBeGreaterThan(0);
  });

  it('drops oldest when retry buffer overflows', async () => {
    const pool = { query: vi.fn(async () => { throw new Error('pg down'); }) };
    const persistence = new AccountPersistence({ pool: pool as any, retryMax: 2 });
    await persistence.upsertPosition({ ...p1, id: 'a' });
    await persistence.upsertPosition({ ...p1, id: 'b' });
    await persistence.upsertPosition({ ...p1, id: 'c' });
    expect(persistence.queueSize()).toBe(2);
  });
});
